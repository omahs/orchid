...
class SymbolTable<T> {
  ...
  /// Use [create] instead.
  @deprecated
  Variable<T> add(String name, {required T value, required bool constant}) {
  ...
  Variable<T>? remove(String name) {
    ...
    while (search != null) {
      Variable<T>? variable;
      try {
        variable = search._variables.firstWhere((v) => v.name == name);
      } catch (err) {
        variable = null;
      }
  ...
  Variable<T>? resolve(String name) {
    var v = _lookupCache.putIfAbsent(name, () {
      Variable<T>? variable;
      try {
        variable = _variables.firstWhere((v) => v.name == name);
      } catch (err) {
        variable = null;
      }
